...
def test_feasible_direction_accepts_first_trial():
    point = _point()
    a, w = _data()
    s = 1e-6
    r = 10.0

    result = armijo_feasible_direction(point, a, w, r=r, s=s, beta=0.5, sigma=0.25)
    expected_direction = feasible_set_projection(point, tuple(-s * block for block in grad(point, a, w)), r)
    ...
    assert all(torch.allclose(actual, expected) for actual, expected in zip(result.step, expected_direction))
    assert result.armijo_lhs + 1e-12 >= result.armijo_rhs


def test_feasible_direction_accepts_with_each_retraction():
    point = _point()
    a, w = _data()
...
def test_feasible_direction_backtracks_on_alpha_without_reprojecting(monkeypatch):
    point = _point()
    a, w = _data()
    projection_calls = 0
    ...
    result = armijo_feasible_direction(point, a, w, r=10.0, s=1e-3, beta=0.5, sigma=0.25)
...
def test_feasible_direction_zero_step_returns_stationary_result():
    ...
    result = armijo_feasible_direction(point, a, w, r=10.0, s=1.0, beta=0.5, sigma=0.25)
...
def test_feasible_direction_returns_failure_when_max_backtracks_exceeded(monkeypatch):
    point = _point()
    a, w = _data()
