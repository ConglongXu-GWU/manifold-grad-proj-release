...
OUTPUT_NAME = "regularized_qr_summary.md"


def load_records(raw_dir: Path, pattern: str = DEFAULT_PATTERN):
    """Load regularized-WLRA QR grid-search JSON records from old or new raw layouts."""
...
def write_summary(
    *,
    ...
    """Write a markdown summary for regularized-WLRA QR raw JSON files."""
    del timestamp
    return _write_summary(
        ...
        title="Regularized WLRA QR Grid Search Summary",
...
def parse_args():
    return build_parser(
        "Summarize regularized-WLRA QR grid JSON records as markdown.",
