...
MAX_BACKTRACKS = 200
DEFAULT_RAW_DIR = REPO_ROOT / "results" / "raws" / "armijo" / "projection_arc" / "qr"

Point = tuple[torch.Tensor, torch.Tensor, torch.Tensor]


@dataclass(frozen=True)
class ProjectionArcQrConfig:
    beta: float
    sigma: float
    initial_step: float
...
def build_grid(
    *,
...
) -> list[ProjectionArcQrConfig]:
    """Build the projection-arc QR grid over beta, sigma, and s_bar."""
    ...
    return [
        ProjectionArcQrConfig(beta=beta, sigma=sigma, initial_step=initial_step)
...
def run_config(
    ...
    *,
    ...
    """Run one projection-arc QR config and return a JSON-serializable record."""
...
def run_grid_search(
    *,
    ...
    """Run the projection-arc QR grid and write timestamped per-config JSON files."""
    if k <= 0:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Timestamped raw JSON grid search for projection-arc Armijo with QR retraction.")
