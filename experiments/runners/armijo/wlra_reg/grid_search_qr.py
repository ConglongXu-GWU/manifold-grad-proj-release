...
MAX_BACKTRACKS = 200
DEFAULT_RAW_DIR = REPO_ROOT / "results" / "raws" / "armijo" / "wlra_reg" / "qr"

Point = tuple[torch.Tensor, torch.Tensor, torch.Tensor]


@dataclass(frozen=True)
class WlraRegQrConfig:
    beta: float
    sigma: float
    initial_step: float
...
def build_grid(
    *,
...
) -> list[WlraRegQrConfig]:
    """Build the regularized-WLRA QR grid over beta, sigma, and s."""
    ...
    return [
        WlraRegQrConfig(beta=beta, sigma=sigma, initial_step=initial_step)
...
def run_config(
    ...
    *,
    ...
    """Run one regularized-WLRA QR config and return a JSON-serializable record."""
    ...
    final_data_loss = wlra_loss(point, a_masked, w)
    final_loss = wlra_loss_reg(point, a_masked, w, lmbda)
    ...
    record = {
        ...
        "final_data_loss": _float(final_data_loss),
...
def run_grid_search(
    *,
    ...
    """Run the regularized-WLRA QR grid and write timestamped per-config JSON files."""
    if k <= 0:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Timestamped raw JSON grid search for regularized WLRA Armijo with QR retraction."
    )
